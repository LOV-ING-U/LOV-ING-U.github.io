...
## 2. Index  
앞선 쿼리를 분석하면서, **인덱스 쓰면 단순히 성능 좋아져요~ 수준으로 이해하면 안된다는 것을 알게 되었을 것이다.** 본인이 작성한 쿼리와, 본인이 제공하는 서비스의 성격과 API 우선순위에 따라 인덱스 도입 유무는 달라질 수 있다.  
  
우선 인덱스가 어떤 방식으로 동작하는지 보자.  
  
(메모리버퍼풀) mysql도 바보가 아니다. 디스크 접근안한다.
(페이지 단위) + 인덱스 도입시 어떻게 되는가?
