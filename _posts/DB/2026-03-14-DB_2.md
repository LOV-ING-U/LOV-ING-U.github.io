...
- **Secondary Index를 수정**하는데, 만약 해당 인덱스가 non-unique secondary index이고 change buffer 활성화 되어있을때 : 메모리 버퍼풀에 **있으면** 바로 수정 + Redo Log 기록하고 / **없으면** Change Buffer에 변경사항을 기록 + Redo Log 기록한다.  
...
- **Secondary Index에 대해 반영**한다. 만약 해당 인덱스가 non-unique secondary index이고 change buffer 활성화 되어있을때 : 메모리에 페이지가 있으면 = 즉시 삭제 마킹 & Redo Log 기록 / 없으면 = Change Buffer에 삭제 마킹 기록 & Redo Log 기록  
...
- **Secondary Index에 대해 반영**한다. 만약 해당 인덱스가 non-unique secondary index이고 change buffer 활성화 되어있을때 : 메모리에 페이지가 있으면 = 기존 값 삭제 마킹 + 새 값 삽입 + Redo Log 기록 / 메모리에 페이지가 없으면 = Change Buffer에 삭제 마킹 + 새 값 삽입 요청 + Redo Log 기록  
