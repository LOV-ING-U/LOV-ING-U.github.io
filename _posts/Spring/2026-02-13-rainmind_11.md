...
수치적 요약 및 스크립트/사용한 PromQL 쿼리는 맨 아래에 별도 첨부한다.  
...
왜냐하면 findAllByUserId의 반환형은 List<엔티티> 인데, JPA의 영속성 컨텍스트에 프록시 객체로 list가 채워진다. 그러나 여기에 .size로 list를 참조하게 되면, 실제 객체들을 가져오게 되어 메모리 사용량과 응답시간 측면에서 병목이 될 수 있다.  
...
- 애초에 countByUserId()를 사용하지 않고, Redis 단일 스레드를 활용한 개선:  
