...
특이사항: ~~race condition 발생 가능. 내부의 SELECT문과 외부의 INSERT 문이 원자적 실행이 보장되지 않는다.~~ race condition은 **발생하지 않는다**. 다만 **deadlock에 매우 취약하여 권장되는 방식은 아닐 것으로 판단된다**, 그렇지만 이에 따른 Error 발생으로 인한 재시도 로직/혹은 에러 메시지 출력을 따로 구축해야 하는 추가적인 과제가 발생한다(2026/02/23 수정).  
